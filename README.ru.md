...
- ⚛️ **Родные адаптеры** — `wmkit/react`, `wmkit/vue`, `wmkit/svelte`, `wmkit/solid`, тонкий сахар над одним ядром
...
```js
import { popout, isPopoutSupported } from 'wmkit/popout'
...
Подключите `wmkit/themes/glass.css` и переопределяйте CSS-переменные (`--wm-radius`, `--wm-bg`, `--wm-accent`, …) — или не подключайте ничего и стилизуйте `data-wm-stage`, `data-wm-focused`, `data-wm-dragging`, `[hidden]` сами.
